...
ATLAS = {"columns": 8, "rows": 7, "cell_width": 192, "cell_height": 208}
ATLAS["width"] = ATLAS["columns"] * ATLAS["cell_width"]
ATLAS["height"] = ATLAS["rows"] * ATLAS["cell_height"]

ROWS = [
    ("sleep", 0, 6, "low-power sleeping loop"),
    ("idle", 1, 6, "neutral breathing and full blink loop"),
    ("busy", 2, 8, "energetic running or working-in-progress loop"),
    ("attention", 3, 6, "jump up, alert, and settle loop"),
    ("completed", 4, 8, "celebration and happy success loop"),
    ("dizzy", 5, 6, "dizzy wobble with spiral eyes loop"),
    ("heart", 6, 6, "affection loop with floating hearts"),
]

TRANSPARENCY_ARTIFACT_RULES = [
    "Prefer pose, expression, and silhouette changes over decorative effects.",
    "Effects are allowed only when they are state-relevant, opaque, hard-edged, pixel-style, fully inside the same frame slot, and small enough to remain readable at 192x208.",
    "Allowed state effects can include a small exclamation mark for attention, compact confetti near the pet for completed, tiny stars touching or overlapping the pet during dizzy, and small hearts near the pet for heart.",
    "Keep effects close to the pet. Do not create large detached sprite components, distant symbols, falling tear drops, separated smoke clouds, loose dust, disconnected outline bits, or stray pixels.",
...
STATE_REQUIREMENTS = {
    "sleep": [
        "Show a quiet low-power sleep state with eyes closed, relaxed body, and very gentle breathing.",
        "Keep the pet mostly planted and compact. Small head, ear, tail, or body movement is enough.",
        "Do not add beds, pillows, moon icons, Z text, speech bubbles, scenery, or separate props unless already part of the pet identity.",
        "The first and last frames should be very close visually so the loop feels calm.",
    ],
    "idle": [
        "CRITICAL: idle is the low-distraction baseline state and the first frame is also used as the reduced-motion static pet.",
        "Include a complete readable blink cycle: open eyes, eyelids lowering, fully blinked or nearly blinked, reopening, then calm open eyes.",
        "Use only subtle idle motion around the blink: gentle breathing, a slight head or body bob, a very small material sway, or another quiet motion that fits the pet persona.",
    ...
    "busy": [
        "Show busy work as a running or quick-trotting loop with clear limb/body cycling and determined forward energy.",
        "The pet may face left or slightly three-quarter left to match the StickS3 busy mascot feel.",
        "Keep the pet centered in each frame. The animation should imply motion without traveling across the slot.",
        "Do not draw speed lines, dust clouds, floor shadows, motion trails, blurred limbs, or detached motion effects.",
    ],
    "attention": [
        "Show attention as a quick jump up, alert pose, then return and settle.",
        "Use face and posture to make the alert moment readable: widened eyes, ears/limbs up, head lifted, or similar persona-preserving reaction.",
        "A small hard-edged exclamation mark may appear close to the pet during the alert frames, fully inside the same frame slot.",
        "Do not draw speech bubbles, UI panels, text labels, large detached punctuation, impact marks, dust, shadows, or motion smears.",
        "Keep the background outside the pet perfectly flat chroma key with no darker key-colored patches.",
    ],
    "completed": [
        "Show completion as celebration and joy: delighted face, raised limbs, bounce, victory pose, or happy spin while keeping the same identity.",
        "Add compact hard-edged confetti pieces falling close around or overlapping the pet in the middle frames.",
        "Confetti must stay inside each frame slot, use non-chroma-key colors, and remain sparse enough for clean transparency extraction.",
        "Do not draw fireworks, glows, large loose sparkles, UI checkmarks, text, banners, scenery, shadows, or background patches.",
    ],
    "dizzy": [
        "Show dizziness through spiral or unfocused eyes, wobbling body, head tilt, and unsteady posture.",
        "Tiny hard-edged stars may touch or overlap the head/body during the dizzy frames, but keep them close and contained.",
        "The pet should recover enough at the end to loop without a harsh pop.",
        "Do not draw large orbiting rings, halos, motion arcs, loose stars far from the pet, red X marks, floor marks, or blurred smears.",
    ],
    "heart": [
        "Show affection through a warm happy pose, soft bounce, blush, heart eyes, or hands/paws close to the face while preserving the pet identity.",
        "Small hard-edged hearts may rise close around the pet, fully inside each frame slot.",
        "Keep hearts sparse, opaque, and pixel-style so transparent cleanup stays reliable.",
        "Do not draw glow, aura, halos, large detached icon clouds, speech bubbles, text, scenery, shadows, or background patches.",
    ],
}
...
def base_pet_prompt(args: argparse.Namespace) -> str:
    ...
    return f"""Create a single clean reference sprite for a Codex Usage Stick digital pet named {args.display_name}.
...
def row_prompt(
    ...
    return f"""Create a single horizontal sprite strip for the Codex Usage Stick digital pet `{args.pet_id}` in the state `{state}`.
